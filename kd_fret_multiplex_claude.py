...
Raw ND2 default (`--nd2-alignment sift`): virtual TIFF stacks are written per FOV,
ImageJ aligns them with Linear Stack Alignment (SIFT), the user draws a single
global laser ROI (post-bleach frame, same heuristic as multiplex TIFF),
`register_and_crop` applies buffered crop outputs under `01_registered/`, Cellpose
and quantify_raw_job consume those cropped stacks unchanged.

Alternative (`--nd2-alignment none`): legacy behaviour—Python ROI cropping before
Cellpose and mask expansion during quantification.
"""
...
class RawFovJob:
    ...
    cellpose_frame_path: Path
    cellpose_mask_path: Path
    # NEW: bounding box (row_start, col_start, row_end, col_end) in full-image
    # coordinates when alignment is Python-cropped (alignment none).
...
def prompt_raw_roi_masks(
    ...
    """
    Open an ImageJ window showing a plain post-bleach donor image, let the user
    ...
    ensure_dir(roi_dir)
    preview_path = roi_dir / "roi_prompt_post_bleach.tif"
...
rename("Post-bleach donor - {measurement_name}");
run("Enhance Contrast", "saturated=0.35");
run("Brightness/Contrast...");
setTool("oval");
waitForUser("Bleached ROI", "This is the post-bleach donor image for {measurement_name}.\\nDraw the bleached laser area, then press OK.");
roiManager("Reset");
roiManager("Add");
roiManager("Select", 0);
...
"""
    ...
    if ask_unbleached:
        macro += f"""
setTool("oval");
waitForUser("Unbleached ROI", "This is the post-bleach donor image for {measurement_name}.\\nDraw the unbleached comparison area, then press OK.");
roiManager("Reset");
roiManager("Add");
roiManager("Select", 0);
...
# --------------------------------------------------------------------------------------
# NEW: precompute_measurement_roi — called BEFORE Cellpose, once per measurement
# --------------------------------------------------------------------------------------
...
def precompute_measurement_roi(
    ...
    This function is called once per measurement at the top of build_raw_jobs(),
    BEFORE any per-FOV Cellpose frames are written.  It populates RAW_ROI_CACHE
    ...
    """
    # ---- fast path: already cached (e.g. shared --laser-roi across measurements) ----
    # We need the image shape first; load the very first frame of the donor stack.
    ...
    # ---- build preview image (mean of sample donor frames) ----
    ...
    # ---- compute masks depending on roi_mode ----
...
# --------------------------------------------------------------------------------------
# Cellpose input creation (CHANGED: accepts crop_rect)
# --------------------------------------------------------------------------------------
...
def make_cellpose_input(
    ...
    """
    Write the donor_pre_mean TIFF that Cellpose will segment.

    CHANGED: if *crop_rect* is provided the image is sliced to
    (row_start:row_end, col_start:col_end) before being saved, giving Cellpose
    a much smaller image and dramatically reducing CPU runtime.
    """
...
def materialize_nd2_fov_ij_stack(
    ...
    dest_measurement_dir.mkdir(parents=True, exist_ok=True)
...
def build_raw_jobs_python_crop(args: argparse.Namespace) -> List[RawFovJob]:
    ...
    CHANGED: precompute_measurement_roi() is called once per measurement before
    any per-FOV frames are written, so every frame is saved already cropped.
    Each RawFovJob stores the crop_rect used so quantify_raw_job() can expand
    the Cellpose mask back to full-image coordinates.
    """
    ...
    for measurement_index, (measurement_name, measurement_dir) in enumerate(measurement_dirs):
        ...
        # NEW: Precompute ROI + crop rect for this measurement BEFORE writing any
        # Cellpose input frames.  For prompt modes this opens ImageJ exactly once.
        # The result is stored in RAW_ROI_CACHE so quantify_raw_job() is free.
        ...
        for position_index in range(position_count):
            ...
            job = RawFovJob(
                ...
                crop_rect=crop_rect,  # NEW: stored so quantify_raw_job can expand the mask
                registered_stack_dir=None,
            )
            # CHANGED: pass crop_rect so the frame is saved already cropped
...
def build_raw_jobs_sift(args: argparse.Namespace) -> List[RawFovJob]:
    ...
    for measurement_index, (measurement_name, measurement_dir) in enumerate(measurement_dirs):
        ...
        cfg_frame_index = ij_cfg.cellpose_frame_index
        ...
        for position_index in range(position_count):
            ...
            seq_files = stack_frame_files(registered_dir)
            if not seq_files:
                raise FileNotFoundError(
                    f"{measurement_name} FOV {fov_disp}: no registered TIFF planes in {registered_dir}"
                )
            if cfg_frame_index < 1 or cfg_frame_index > len(seq_files):
                raise ValueError(
                    f"{measurement_name} FOV {fov_disp}: cellpose_frame_index "
                    f"{cfg_frame_index} outside [1, {len(seq_files)}] for {registered_dir}"
                )
            frame_path = cellpose_dir / f"{safe_measurement}_{fov_disp}_donor_pre_mean.tif"
            mask_path = frame_path.with_name(frame_path.stem + "_cp_masks.tif")
            shutil.copyfile(seq_files[cfg_frame_index - 1], frame_path)
...
def make_region_masks(
    ...
    CHANGED: always checks RAW_ROI_CACHE first (populated by
    precompute_measurement_roi during build_raw_jobs).  For all three roi_modes
    the cache will normally be warm, so this function becomes a fast lookup.
    If the cache is cold (e.g. --skip-cellpose rerun without rerunning
    build_raw_jobs), it falls back to on-demand computation as before.
    """
    mask_sidecar = (
    ...
    # Cache miss — compute on demand (fallback for reruns / edge cases)
    logging.debug(
    ...
    if args.roi_mode in {"prompt-ring", "prompt-two"}:
        roi_dir = ensure_dir(
            ...
            / re.sub(r"[^A-Za-z0-9_.-]+", "_", job.measurement_name)
        )
...
def quantify_raw_job(job: RawFovJob, args: argparse.Namespace) -> pd.DataFrame:
    ...
    CHANGED: stacks are loaded first to obtain *image_shape*, then the Cellpose
    mask is loaded and — if the Cellpose frame was cropped (job.crop_rect is not
    None) — expanded back to full-image coordinates via expand_cropped_mask().
    All downstream overlap, background, and FRET calculations then operate on
    full-size arrays, so no other code needed to change.
    """
    # 1. Load stacks in quantification coordinates (full uncropped ND2, or cropped registered TIFF).
        ...
        records.append(
            {
                ...
                "crop_rect": str(job.crop_rect),   # NEW: audit trail
                **counts,
